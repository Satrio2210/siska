...
<?php
  $rawinput = $_POST['q'];
  list($kata,$subunit) = explode("|",$rawinput);
  //$kata = 'x';
  if (strlen($kata) > 0)
  {
  $xquery = "SELECT TBLI_UNIT_CODE, TBLI_UNIT_NAME 
              FROM tbliunit 
              WHERE TBLI_UNIT_DEVI = 1 AND TBLI_UNIT_NAME LIKE '$subunit%' AND TBLI_UNIT_STAT = 'Y'
