...
if (ISSET($_GET['regicode']))
{
	...
	$query_header = "SELECT TRXA_REGI_CODE AS REGI_CODE, TRXA_PATI_CODE AS PATI_CODE, 
					...
					(SELECT CONCAT(TRXA_ENTR_DATE,' ',TRXA_ENTR_TIME) FROM trxasale 
					WHERE TRXA_REGI_CODE=REGI_CODE ORDER BY TRXA_ENTR_DATE DESC,TRXA_ENTR_TIME DESC LIMIT 1) AS INVOICE_DATE, 
					...
					TRXA_REGI_DOCT, 
					(SELECT PASS_USER_NAME FROM passiden WHERE PASS_USER_IDEN=TRXA_REGI_DOCT) AS PRIMARY_DOCTOR 
	...
	$pdf->Cell(8,7,' ','LR',0,'C'); 
	$pdf->Cell(50,7,'DRUGS','L',0,'L'); 
	$pdf->Cell(30,7,'  ','R',0,'L'); 
	$pdf->Cell(10,7,' ','LR',0,'R'); 
	$pdf->Cell(15,7,' ','LR',0,'L'); 
	$pdf->Cell(25,7,' ','LR',0,'R');
	$pdf->Cell(25,7,' ','LR',0,'R'); 
	$pdf->Cell(25,7,' ','LR',1,'R');
	...
	$query_action = "SELECT TRXA_TRET_CODE, TRXA_MEDI_CODE, 
              ...
              AND TRXA_TRET_CODE = '$regicode' AND TRXA_TRET_STAT = 'P' AND TRXA_VIEW_STAT='Y'";
	...
	$pdf->Cell(8,7,' ','LR',0,'C'); 
	$pdf->Cell(50,7,'CONSUMABLE','L',0,'L'); 
	$pdf->Cell(30,7,'  ','R',0,'L'); 
	$pdf->Cell(10,7,' ','LR',0,'R'); 
	$pdf->Cell(15,7,' ','LR',0,'L'); 
	$pdf->Cell(25,7,' ','LR',0,'R');
	...
	$query_csbl = "SELECT TRXA_CSBL_CODE, TRXA_STOCK_CODE, 
              ...
              FROM trxacsbl WHERE TRXA_CSBL_CODE = '$regicode' AND TRXA_CSBL_STAT = 'P' AND TRXA_VIEW_STAT='Y'";
	...
	$pdf->Cell(113,5,'SUB TOTAL :','T',0,'R'); 
	$view_sub_total = number_format($sub_total, 0, '', '.');
	$pdf->Cell(25,5,''.$view_sub_total.'','T',0,'R');
	$pdf->Cell(25,5,'0','T',0,'R'); 
	$pdf->Cell(25,5,''.$view_sub_total.'','T',1,'R');

	$pdf->Cell(113,5,'DISCOUNT :',0,0,'R'); 
	$pdf->Cell(25,5,'',0,0,'R');
	$pdf->Cell(25,5,'',0,0,'R'); 
	$pdf->Cell(25,5,'0',0,1,'R');

	$pdf->Cell(113,5,'ADMIN CHARGE :',0,0,'R'); 
	$pdf->Cell(25,5,'',0,0,'R');
	$pdf->Cell(25,5,'',0,0,'R');
...
// Periksa apakah ada obat racikan
	...
	$pdf->Cell(25,5,''.$view_fee_admin.'',0,1,'R');

	$pdf->Cell(113,5,'TOTAL :',0,0,'R'); 
	$pdf->Cell(25,5,'',0,0,'R');
	$pdf->Cell(25,5,'',0,0,'R'); 
	$sub_total = $sub_total + $total_admin;
	$view_sub_total = number_format($sub_total, 0, '', '.'); 
	$pdf->Cell(25,5,''.$view_sub_total.'',0,1,'R');
	...
	$pdf->Cell(113,5,'BALANCE :',0,0,'R'); 
	$pdf->Cell(25,5,'',0,0,'R');
	$pdf->Cell(25,5,'',0,0,'R'); 

	$view_balance = number_format($balance, 0, '', '.');
	$pdf->Cell(25,5,''.$view_balance.'',0,1,'R');
    ...
    // Periksa apakah ada pembayaran kwitansi
    $periksakwitansi = "SELECT COUNT(*) FROM trxasale WHERE TRXA_SALE_CODE='$invoice_no' AND TRXA_VIEW_STAT='Y'";
    $periksakwitansi_di_query=$db->query($periksakwitansi) or die ("Gagal Periksa Kwitansi");
    $ketersediaan_kwitansi = $periksakwitansi_di_query->fetchColumn();

    if ($ketersediaan_kwitansi == 0)
    {
	$pdf->Cell(18,8,'No Payment','LTBR',0,'L'); 
	$pdf->Cell(18,8,' ','LTBR',0,'L'); 
	$pdf->Cell(22,8,' ','LTBR',0,'L'); 
	$pdf->Cell(25,8,' ','LTBR',0,'R'); 
	$pdf->Cell(25,8,' ','LTBR',0,'L'); 
	$pdf->Cell(30,8,' ','LTBR',0,'R');
	$pdf->Cell(25,8,' ','LTBR',0,'R'); 
	$pdf->Cell(25,8,' ','LTBR',1,'R');

	$pdf->Cell(163,8,'TOTAL :',0,0,'R'); 
	$pdf->Cell(25,8,' 0 ',0,1,'R');
