...
<?php
  ...
  $xquery = "SELECT CONCAT(COAC_MAST_PRNT,'.', COAC_MAST_CODE) AS MAST_CODE, COAC_MAST_PRNT, COAC_MAST_NAME,   
             ...
             FROM coacmast 
             WHERE COAC_MAST_PRNT IN ('5.1','5.2') AND COAC_VIEW_STAT='Y' AND COAC_MAST_STAT = 'C'
             ORDER BY MAST_CODE";            
  }
  else
  {
  $xquery = "SELECT CONCAT(COAC_MAST_PRNT,'.', COAC_MAST_CODE) AS MAST_CODE, COAC_MAST_PRNT, COAC_MAST_NAME, 
             ...
             AND COAC_MAST_PRNT IN ('5.1','5.2') AND COAC_VIEW_STAT='Y' AND COAC_MAST_STAT = 'C'
             OR COAC_MAST_NAME LIKE '%$kata%' 
             AND COAC_MAST_PRNT IN ('5.1','5.2') AND COAC_VIEW_STAT='Y' AND COAC_MAST_STAT = 'C'
...
echo '<tr>';
echo '<td style="width: 100px;" onClick="isipayecode(\''.$coaccode.'\',\''.$coacname.'\');" 
      style="cursor:pointer">'.$k['MAST_CODE'].'</td>';
echo '<td style="width: 200px;" onClick="isipayecode(\''.$coaccode.'\',\''.$coacname.'\');" 
