...
while ($k = $q->fetch(PDO::FETCH_ASSOC))
{
  ...
  $outunitdevi = $k['TBLI_UNIT_DEVI'];

  $periksaunit=strpos($outunitname,"@");

  if ($periksaunit)
      {
        list($partunit,$subpartunit) = explode("@",$outunitname);
      }
      else 
      {
          $subpartunit = $outunitname; 
      }


echo '<tr>';
echo '<td style="width: 100px;" onClick="isiunitcode(\''.$outunitcode.'\',\''.$outunitname.'\',\''.$subpartunit.'\');" 
      style="cursor:pointer">'.$k['TBLI_UNIT_CODE'].'</td>';
echo '<td style="width: 200px;" onClick="isiunitcode(\''.$outunitcode.'\',\''.$outunitname.'\',\''.$subpartunit.'\');" 
