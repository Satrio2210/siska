  ...
  if (aksesku.readyState==4)
    ...
    if(data.length>1)
      ...
      {
        document.getElementById('txtsgnacode').setAttribute('disabled','true');
        document.getElementById('txtsgnaname').setAttribute('disabled','true');
    ...
    if(data.length>1)
      {
        ...
        swal({
            title: 'Code is Exist' ,
            text: 'Kode Signa sudah ada',
            icon: 'warning',
        });
      }
      else
      {
        document.getElementById("txtsgnaname").removeAttribute('disabled');
...
// end periksa Kode
  ...
  var ajaxinput;
  function input(sgnacode,sgnaname)
  {
    ajaxinput = buatajaxinput();
    var url="TRXAPOLI05E.php";
    ajaxinput.onreadystatechange=stateChangedInput;
    var params = "q="+sgnacode+"|"+sgnaname;
    //alert(params);
...
var ajaxview
function viewcode(sgnacode)
{
  try 
  {
    ajaxview = buatajaxview();
    var url="TRXAPOLI05C.php";
    ajaxview.onreadystatechange=stateChangedView;
    var params = "q="+sgnacode;
    //alert(params);
  ...
  if (ajaxview.readyState==4)
    {
      ...
      {
      var res = data.split("|");
      document.getElementById("txtsgnacode").value = res[1];
      document.getElementById("txtsgnaname").removeAttribute('disabled');
      document.getElementById("txtsgnaname").value = res[2];
      }      
        //}
    }
  }
...
// Hapus Transportasi di Tabel tblitrans
  var ajaxhapus;
  function hapuscode(sgnacode)
  {
    ajaxhapus = buatajaxhapus();
    var url="TRXAPOLI05D.php";
